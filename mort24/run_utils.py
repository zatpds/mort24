...
from mort24.models.utils import JsonResultLoggingEncoder
from mort24.wandb_utils import wandb_log
import polars as pl
...
def build_parser() -> ArgumentParser:
    ...
    parser.add_argument("-m", "--model", default="Transformer", help="Name of the model gin.")
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Path to a pretrained imputation model to use for imputing missing values.",
    )
    return parser
...
def load_pretrained_imputation_model(model_path: str = None):
    """Load a pretrained imputation model from file.

    Args:
        model_path: Path to the pretrained imputation model file.

    Returns:
        Loaded imputation model or None if no path specified.
    """
    ...
    if not model_path.exists():
        logging.error(f"Pretrained imputation model not found at {model_path}")
        return None
    
    try:
        model = torch.load(model_path)
        logging.info(f"Loaded pretrained imputation model from {model_path}")
        return model
    except Exception as e:
        logging.error(f"Failed to load pretrained imputation model: {e}")
        return None